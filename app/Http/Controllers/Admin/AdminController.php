...
class AdminController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'avatar' => 'nullable|image|mimes:png,jpg,jpeg,gif,webp'
        ]);
        $admin = new Admin;
        if( $request->avatar ){
            Image::make($request->avatar)->save('uploads/admins/' . $request->avatar->hashName());
    ...
    public function update(Request $request, Admin $admin)
    {
        ...
        if( $request->avatar ){
            if( $admin->avatar != 'uploads/admins/default.png' && file_exists($admin->avatar)){
                unlink($admin->avatar);
            }
            Image::make($request->avatar)->save('uploads/admins/' . $request->avatar->hashName());
            $admin->avatar = 'uploads/admins/' . $request->avatar->hashName();
